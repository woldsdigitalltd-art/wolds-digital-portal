...
export default async function WebsiteDashboardPage({ params }: PageProps) {
  ...
  const audit        = seoMap.get(site.id)         ?? null
  const pageSpeed    = pageSpeedMap.get(site.id)   ?? null
  const brokenLinks  = brokenLinksMap.get(site.id) ?? null
    ...
    <div>
      ...
      {!anyIntegration ? (
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
          {hasMonitor      && <MonitoringCard   siteId={site.id} uptime={uptime}       />}
          {hasSeo          && <SeoCard          siteId={site.id} audit={audit}         />}
          {hasPageSpeed    && <PageSpeedCard    siteId={site.id} report={pageSpeed}    />}
          {hasBrokenLinks  && <BrokenLinksCard  siteId={site.id} report={brokenLinks}  />}
        </div>
      )}
    </div>
  )
...
function MonitoringCard({
  ...
  return (
    <CardShell
      href={`/portal/websites/${siteId}/monitoring`}
      eyebrow="Monitoring"
      title={cfg.label}
      titleClass={cfg.text}
    >
...
function SeoCard({
  ...
  if (!audit) {
    return (
      <CardShell
        href={`/portal/websites/${siteId}/seo`}
        eyebrow="SEO"
        title="Audit pending"
  ...
  return (
    ...
    >
      <div className={`flex h-12 w-12 shrink-0 flex-col items-center justify-center rounded-xl ${colour.bg} ring-1 ${colour.ring}`}>
        <span className={`text-base font-bold leading-none ${colour.text}`}>{audit.score}</span>
...
function PageSpeedCard({
  ...
  if (!report) {
    return (
      <CardShell
        href={`/portal/websites/${siteId}/performance`}
        eyebrow="Performance"
        title="Audit pending"
        titleClass="text-navy-700"
      >
...
function BrokenLinksCard({
  ...
  if (!report) {
    return (
      <CardShell
        href={`/portal/websites/${siteId}/broken-links`}
